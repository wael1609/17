...
class Database {
    ...
    public function getUserInformation($userId) {
        $sql = "SELECT * FROM utilisateurs WHERE idu = $userId";
...
public function getAllFeedbacks() {
    $sql = "SELECT * FROM feedback ORDER BY created_at DESC"; // Changed table name to 'feedback'
    $result = $this->conn->query($sql);
    $feedbacks = array();
    if ($result && $result->num_rows > 0) {
    ...
    public function getAllMaintenanceInfo() {
        $sql = "SELECT * FROM car_parts_declarations";
        $result = $this->conn->query($sql);
        $declarations = array();
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
...
public function addFeedback($userId, $feedback) {
    // Prepare SQL query
...
// Method to retrieve all lavage requests from the database
public function getAllLavageRequests() {
    $sql = "SELECT * FROM lavage"; // Assuming 'lavage' is the table name
    $result = $this->conn->query($sql);
    $lavageRequests = array();
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $lavageRequests[] = $row;
        }
    }
    return $lavageRequests;
}
   // Method to mark a lavage request as done in the database
   public function markLavageRequestAsDone($lavageRequestId) {
    // Assuming 'lavage' is the table name and 'done' is the column representing the status of the request
    $sql = "UPDATE lavage SET done = 1 WHERE id = $lavageRequestId";
    $this->conn->query($sql);
}
    ...
    // Method to insert a lavage request into the database
    public function insertLavageRequest($userId, $requestedDate) {
        $stmt = $this->conn->prepare("INSERT INTO lavage (idu, date, done, status) VALUES (?, ?, 0, 'pending')");
        $stmt->bind_param("is", $userId, $requestedDate);
        $result = $stmt->execute();
        if ($result) {
            return true; // Lavage request inserted successfully
        } else {
            return false; // Failed to insert lavage request
        }
    }
